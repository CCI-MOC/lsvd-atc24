...
\peter{
  Push back a little on the original work. The original is a bit of an ivory
  tower approach, this is bringing it to the real world.

  Problems: benchmarks some didn't have GC running, a lot of speed came from
  having a kernel module, which in real workloads would be one extra hop instead
  of one fewer

  Another approach may be to highlight the shared cache as a benefit of moving
  lsvd away from the client
  }
